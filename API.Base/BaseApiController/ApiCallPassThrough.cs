...
namespace MapHive.Server.Core.API
{
    public abstract partial class BaseApiController
    {
        ...
        protected class ApiCallOutput<TOut>
        {
            /// <summary>
        ...
        /// <returns></returns>
        protected IHttpActionResult ApiCallPassThrough(IRestResponse apiResponse)
        {
                ...
                if (contentType == "application/json")
                {
                    return Content(
                        ...
                        string.IsNullOrEmpty(content)
                            ? (object)(apiResponse.RawBytes ?? Array.Empty<byte>())
                            : !string.IsNullOrEmpty(content)
            ...
            return new NegotiatedContentResult<object>((HttpStatusCode)(int)apiResponse.StatusCode, content, this);
        ...
        /// <returns></returns>
        protected async Task<IRestResponse> RestApiCall(string url, string route,
        ...
        /// <returns></returns>
        protected async Task<IRestResponse> RestApiCall(HttpRequest origHttpRequest, string url, string route, Method method = Method.GET,
        ...
        /// <param name="customHeaders"></param>
        /// <returns></returns>
        protected async Task<ApiCallOutput<TOut>> RestApiCall<TOut>(string url, string route, Method method = Method.GET,
        ...
        /// <param name="transferRequestHdrs">Whether or not should auto transfer request headers so they are sent out </param>
        /// <returns></returns>
        protected async Task<ApiCallOutput<TOut>> RestApiCall<TOut>(HttpRequest origHttpRequest, string url, string route, Method method = Method.GET,
            Dictionary<string, object> queryParams = null, object data = null, string authToken = null, Dictionary<string, string> customHeaders = null, List<string> headersToSkip = null, bool transferAuthHdr = true, bool transferRequestHdrs = true)
        {
        ...
        public void TransferRequestHeaders(HttpRequest origHttpRequest, RestRequest restRequest, Dictionary<string, string> customHdrs, List<string> headersToSkip, bool transferHdrs)
        {
            ...
            foreach (var hdr in origHttpRequest.Headers.AllKeys)
            {
                if (_headersToIgnoreWhenTransferring.Contains(hdr.ToLower()))
                    continue;

                //assume that custom headers overwrite whatever are the incoming headers
                //this is done outside of this method
                if (customHdrs?.ContainsKey(hdr) == true)
                    continue;

                //any explicit headers to skip?
                if (headersToSkip != null && headersToSkip.Contains(hdr))
                    continue;
                ...
                restRequest.AddHeader(hdr, origHttpRequest.Headers[hdr]);
            }
        }
    }
}
